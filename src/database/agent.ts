...
import type { IntentContext } from "@langgraph-toolkit/core";
import type { ToolkitRuntime } from "@langgraph-toolkit/core/runtime";
import { createMcpGateway } from "@langgraph-toolkit/mcp/advanced";
import { createDatabaseTools, createMemoryGateway } from "./tools.js";
import type {
  McpDatabaseQueryResult,
  McpDatabaseRow,
  McpDatabaseSchema,
} from "./tools.js";
import type { McpGateway, McpServerDeclaration } from "@langgraph-toolkit/mcp";
...
export interface DatabaseMcpState {
  ...
  readonly rows: readonly McpDatabaseRow[];
  readonly citations: readonly string[];
  readonly schema: McpDatabaseSchema | null;
  readonly plan: DatabaseMcpPlan | null;
  readonly validation: DatabaseMcpValidation | null;
  readonly queryResult: McpDatabaseQueryResult | null;
  readonly queryErrors: readonly DatabaseMcpError[];
  readonly repairAttempts: number;
  readonly audit: readonly DatabaseMcpAudit[];
  readonly clarification: DatabaseMcpClarificationRequest | null;
  ...
  readonly answer: DatabaseMcpAnswer | null;
  readonly approved: boolean;
...
function validationFor(state: DatabaseMcpState, global: DatabaseMcpGlobal): DatabaseMcpValidation {
  if (state.plan === null) return { allowed: false, normalizedSql: "", reasons: ["No query plan exists"], policyDecision: "deny", estimatedCost: 0, allowedColumns: [], tenantPredicatePresent: true };
...
function createDatabaseGraph(gateway: McpGateway, global: DatabaseMcpGlobal, name: string) {
  const { schemaTool, executeQueryTool } = createDatabaseTools(gateway, {
  ...
  const nodes = {
    ...
    plan: async (state: DatabaseMcpState, ctx: Context): Promise<Partial<DatabaseMcpState>> => {
      if (state.status === "need_clarification" || state.schema === null) return { status: state.status };
    ...
    repair: async (state: DatabaseMcpState, ctx: Context): Promise<Partial<DatabaseMcpState>> => {
      if (state.plan === null || state.repairAttempts >= global.maxRepairAttempts) return { status: "failed" };
    ...
    retrieve: async (state: DatabaseMcpState, ctx: Context): Promise<Partial<DatabaseMcpState>> => {
      if (state.plan === null || state.validation?.allowed !== true) return { status: "failed" };
    ...
    compose: async (state: DatabaseMcpState, ctx: Context): Promise<Partial<DatabaseMcpState>> => {
      ...
      const answer = state.status === "need_clarification"
        ...
        : state.status === "unauthorized"
          ...
          : state.status === "datasource_unavailable" || state.queryResult === null
    ...
    approval: async (state: DatabaseMcpState, ctx: Context): Promise<Partial<DatabaseMcpState>> => {
      if (!global.approvalRequired || state.answer === null) return { approved: true, status: "completed" };
      ...
      return { approved: false, status: "completed" };
    },
    respond: async (state: DatabaseMcpState): Promise<Partial<DatabaseMcpState>> => ({ status: state.answer === null ? "failed" : "completed" }),
  };

  const approvalGate = gate<DatabaseMcpState, DatabaseMcpInterrupt>("database-answer-approval", async (state) => state.answer === null ? { kind: "deny", reason: "No answer exists to approve." } : { kind: "allow" });
...
export function defineDatabaseGraph(
  ...
  const gateway = options.mcp ?? createMemoryGateway(options.rows ?? []);
...
export async function createDatabaseAgent(options: DatabaseMcpAgentOptions = {}): Promise<DatabaseMcpAgent> {
  ...
  if (gateway === undefined) {
    gateway = createMemoryGateway(options.rows ?? []);
    ownsGateway = true;
  }
